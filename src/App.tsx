...
function App() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const sellUnits = ethers.utils.parseUnits(sellStr, pyusdDec).toString()
        ...
        if (!res.ok) {
          ...
          if (res.status === 404) {
            setQuoteError('0x route not available, will use Uniswap fallback')
            return
          }
        ...
        const outHuman = ethers.utils.formatUnits(data.buyAmount, stableOut.decimals)
  ...
  async function handleConfirmDeposit() {
    try {
      ...
      const pyusdDec: number = await pyusd.decimals()
      const totalAmt = Number(depositAmount)
      if (!isFinite(totalAmt) || totalAmt <= 0) throw new Error('Invalid amount')
      ...
      const totalUnits = ethers.utils.parseUnits(totalAmt.toFixed(pyusdDec), pyusdDec)
      const swapUnits = ethers.utils.parseUnits(swapAmt.toFixed(pyusdDec), pyusdDec)
      ...
      if (testMode) {
        console.log('🧪 TEST MODE: Skipping all swaps, using existing balances')
        buyAmount = Math.min(Number(currentUsdcBalance.toString()), Number(swapUnits.toString())).toString()
      }
      // Skip swap if user already has sufficient USDC
      else if (currentUsdcBalance.gte(swapUnits) && swapUnits.gt(0)) {
        console.log('User already has sufficient USDC, skipping all swap logic')
        ...
        try {
          ...
          if (!txTo || !txData || !allowanceTarget || !buyAmount) throw new Error('Invalid 0x swap payload')
          ...
          if (currentAllow.lt(swapUnits)) {
            console.log(`Approving 0x allowanceTarget: current=${currentAllow.toString()}, needed=${swapUnits.toString()}`)
            const appr = await pyusdW.approve(allowanceTarget, swapUnits)
            await appr.wait()
          } else {
            console.log(`0x allowanceTarget already approved: ${currentAllow.toString()} >= ${swapUnits.toString()}`)
          }
          // Execute 0x swap
          const sent = await signer.sendTransaction({ to: txTo, data: txData, value: ethers.BigNumber.from(txValue) })
          await sent.wait()
        } catch (zeroXError) {
          ...
          if (currentAllow.lt(swapUnits)) {
            console.log(`Approving UNISWAP_V3_ROUTER fallback: current=${currentAllow.toString()}, needed=${swapUnits.toString()}`)
            const appr = await pyusdW.approve(UNISWAP_V3_ROUTER, swapUnits)
            await appr.wait()
          } else {
            console.log(`UNISWAP_V3_ROUTER fallback already approved: ${currentAllow.toString()} >= ${swapUnits.toString()}`)
          }
      ...
      for (const fee of feeTiers) {
        try {
          ...
          if (addr !== ethers.constants.AddressZero) {
            poolAddress = addr
            selectedFeeTier = fee
            console.log(`Found PYUSD/USDC pool at ${addr} with ${fee/10000}% fee`)
            break
          }
        } catch (e) {
      ...
      if (poolAddress === ethers.constants.AddressZero) {
        throw new Error('No PYUSD/USDC Uniswap V3 pool found on Arbitrum One. The pool may not exist yet.')
      }
      ...
      try {
        ...
        throw new Error(`Pool contract error for address ${poolAddress}. Error: ${poolError?.message || poolError}`)
      }
      ...
      if (!slot0 || !fee || !token0Addr || !token1Addr) {
        throw new Error('Invalid pool data received. Pool may not be properly initialized.')
      }
      
      // Compute +/-5% range using Uniswap v3 SDK helpers
      const spacing: number = (TICK_SPACINGS as any)[feeTier] ?? (feeTier === 500 ? 10 : feeTier === 3000 ? 60 : 200)
      ...
      // Validate tick range
      if (lower >= upper) {
        throw new Error(`Invalid tick range: lower=${lower}, upper=${upper}, current=${currentTick}`)
      }
      
      console.log(`Tick range: ${lower} to ${upper} (current: ${currentTick}, spacing: ${spacing})`)
      ...
      const hasCorrectTokens = (token0 === pyusdAddr && token1 === usdcAddr) || (token0 === usdcAddr && token1 === pyusdAddr)
      if (!hasCorrectTokens) {
        throw new Error(`Pool ${poolAddress} does not contain PYUSD and USDC. Found tokens: ${token0Addr}, ${token1Addr}`)
      }
      ...
      if (testMode) {
        ...
        const usdcToUse = currentUsdcBalance.gt(0) ? 
          ethers.BigNumber.from(Math.min(Number(currentUsdcBalance.toString()), Number(totalUnits.div(2).toString())).toString()) :
      ...
      else if (currentUsdcBalance.gte(swapUnits) && swapUnits.gt(0) && amountUsdc.eq(0)) {
      ...
      // If no swap occurred or swap failed, we need to do a minimal swap to get both tokens
        ...
        console.log('Forcing minimal swap of', minSwapAmount.toString(), 'PYUSD to get USDC')
        ...
        try {
          ...
          if (currentAllow.lt(minSwapAmount)) {
            console.log(`Approving UNISWAP_V3_ROUTER for forced swap: current=${currentAllow.toString()}, needed=${minSwapAmount.toString()}`)
            const appr = await pyusdW.approve(UNISWAP_V3_ROUTER, minSwapAmount)
            await appr.wait()
          } else {
            console.log(`UNISWAP_V3_ROUTER already approved for forced swap: ${currentAllow.toString()} >= ${minSwapAmount.toString()}`)
          }
          ...
          throw new Error('Unable to create balanced LP position. Both PYUSD and USDC are required.')
        }
      }
      
      // Calculate token amounts based on pool token ordering
      const amount0Desired = token0 === pyusdAddr ? finalAmountPyusd : finalAmountUsdc
      const amount1Desired = token1 === pyusdAddr ? finalAmountPyusd : finalAmountUsdc
      
      // Ensure we have meaningful amounts for LP
      if (amount0Desired.eq(0) && amount1Desired.eq(0)) {
        throw new Error('Cannot create LP position with zero amounts for both tokens')
      }
      ...
      const totalApprovalsNeeded = (needsToken0Approval ? 1 : 0) + (needsToken1Approval ? 1 : 0)
      ...
      if (totalApprovalsNeeded === 0) {
        ...
        if (needsToken0Approval) {
          console.log(`Direct approval for token0 (${token0Addr}): current=${allow0.toString()}, needed=${amount0Desired.toString()}`)
          const approveTx = await token0C.approve(UNISWAP_POSITION_MANAGER, amount0Desired)
          await approveTx.wait()
          console.log('Token0 approval completed')
        } else if (needsToken1Approval) {
          console.log(`Direct approval for token1 (${token1Addr}): current=${allow1.toString()}, needed=${amount1Desired.toString()}`)
          const approveTx = await token1C.approve(UNISWAP_POSITION_MANAGER, amount1Desired)
          await approveTx.wait()
        ...
        if (needsToken0Approval) {
          ...
          const approveCalldata = erc20Interface.encodeFunctionData('approve', [UNISWAP_POSITION_MANAGER, amount0Desired])
          approveCalls.push({
            target: token0Addr,
            allowFailure: false,
        ...
        if (needsToken1Approval) {
          ...
          const approveCalldata = erc20Interface.encodeFunctionData('approve', [UNISWAP_POSITION_MANAGER, amount1Desired])
          approveCalls.push({
            target: token1Addr,
            allowFailure: false,
            callData: approveCalldata
        ...
        const multicall = new ethersLib.Contract(MULTICALL3, MULTICALL3_ABI, signer)
      ...
      const pm = new ethers.Contract(UNISWAP_POSITION_MANAGER, POSITION_MANAGER_ABI, signer)
      const deadline = Math.floor(Date.now() / 1000) + 1800
      
      // Validate mint parameters
      if (amount0Desired.lte(0) || amount1Desired.lte(0)) {
        throw new Error(`Invalid amounts for minting: amount0=${amount0Desired.toString()}, amount1=${amount1Desired.toString()}. Need both PYUSD and USDC for LP.`)
      }
      ...
      try {
        ...
        throw new Error(`Transaction would fail: ${gasError?.reason || gasError?.message || gasError}. Check if you have sufficient token balances and allowances.`)
      }
      ...
      try {
        ...
        if (mintError.receipt) {
          console.log('Transaction was mined but failed. Receipt:', mintError.receipt)
          throw new Error(`Position creation failed: ${mintError.reason || mintError.message}. Transaction hash: ${mintError.receipt.transactionHash}`)
        } else {
          throw new Error(`Position creation failed: ${mintError.reason || mintError.message || mintError}`)
        }
      }
  ...
  async function fetchPositions() {
    ...
    try {
      ...
      const positionManager = new ethers.Contract(UNISWAP_POSITION_MANAGER, POSITION_MANAGER_ABI, provider)
      ...
      for (let i = 0; i < balance.toNumber(); i++) {
        try {
          ...
          const isPyusdUsdcPool = (token0 === pyusdAddr && token1 === usdcAddr) || (token0 === usdcAddr && token1 === pyusdAddr)
          
          if (isPyusdUsdcPool && position.liquidity.gt(0)) {
            console.log(`Found PYUSD/USDC position: tokenId=${tokenId.toString()}`, position)
  ...
  async function handleWithdraw(position: Position) {
    ...
    try {
      ...
      console.log(`🔄 Withdrawing position #${position.tokenId} in single transaction...`)
      ...
      const positionManager = new ethers.Contract(UNISWAP_POSITION_MANAGER, POSITION_MANAGER_ABI, signer)
      const deadline = Math.floor(Date.now() / 1000) + 1800
      ...
      const positionInterface = new ethers.utils.Interface(POSITION_MANAGER_ABI)
      
      // 1. Decrease liquidity to 0
      const decreaseLiquidityData = positionInterface.encodeFunctionData('decreaseLiquidity', [{
        tokenId: position.tokenId,
      ...
      const multicallTx = await positionManager.multicall([decreaseLiquidityData, collectData])
  ...
  return (
    <div className="page">
      ...
      <main className="content">
        ...
        <section>
          <div className="section-head">
            <h3 className="section-title">Your Positions {loadingPositions && <span className="spinner" />}</h3>
            <p className="section-sub">Track your active deposits and manage exits back to PYUSD.</p>
          </div>
          {positions.length === 0 ? (
            <div className="card">
              <p className="feature-desc">{loadingPositions ? 'Loading positions...' : 'No active positions yet. Start by choosing a strategy above and depositing PYUSD via the Zap.'}</p>
            </div>
          ) : (
            <div className="grid">
              {positions.map((p) => (
                <div className="card" key={p.id}>
                  ...
                  <div className="kv">
                    <div className="kv-row"><span className="kv-key">Token ID</span><span className="kv-val">#{p.tokenId}</span></div>
                    <div className="kv-row"><span className="kv-key">Liquidity</span><span className="kv-val">{p.liquidity ? Number(p.liquidity).toLocaleString() : '—'}</span></div>
                    <div className="kv-row"><span className="kv-key">Fee Tier</span><span className="kv-val">{p.fee ? (p.fee / 10000).toFixed(2) + '%' : '—'}</span></div>
                    <div className="kv-row"><span className="kv-key">Tick Range</span><span className="kv-val">{p.tickLower && p.tickUpper ? `${p.tickLower} to ${p.tickUpper}` : '—'}</span></div>
                  </div>
                  <div className="wallet-actions">
                    <button 
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="card route">
                ...
                <div className="seg"><span className="pill">USDC only</span></div>
                ...
                {!testMode && (
                  <div className="quote-row"><span className="k">Auto Swap</span><span className="v">{swapAmount.toFixed(2)} PYUSD → USDC</span></div>
                )}
                {quoteLoading ? (
                  <div className="quote-row"><span className="k">Quote</span><span className="v"><span className="spinner" /> Fetching…</span></div>
                ) : quoteError ? (
                  <div className="quote-row"><span className="k">Route</span><span className="v warning">{quoteError}</span></div>
                ) : (
                  <>
                    <div className="quote-row"><span className="k">0x Quote</span><span className="v">{quoteOut || '—'}</span></div>
                    <div className="quote-row"><span className="k">Estimated Gas</span><span className="v">{quoteGas || '—'}</span></div>
                  </>
                )}
                <div className="quote-row small"><span className="k">Uniswap Pool</span><span className="v mono">Auto-detected from factory</span></div>
                <div className="quote-row small"><span className="k">Position Manager</span><span className="v mono">{UNISWAP_POSITION_MANAGER}</span></div>
                <div className="quote-row small"><span className="k">Factory</span><span className="v mono">{UNISWAP_FACTORY}</span></div>
                <p className="review-note">Auto-calculates optimal swap (≈50% for 1:1 USD ratio). Uses 0x with Uniswap fallback.</p>
              </div>
            </div>
            <div className="modal-actions">
              ...
              {!account ? (
                ...
                <button
                  ...
                  disabled={!depositAmount || Number(depositAmount) <= 0 || txLoading}
                  onClick={() => {
            ...
            {txError && (
              <div className="alert alert-error" role="alert" style={{ margin: '0 18px 12px' }}>
                {txError}
              </div>
            )}
